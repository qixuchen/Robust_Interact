...
// Use LP to find a feasible point of the half space intersection (used later in Qhull for half space intersection)
point_t* find_feasible_active(std::vector<hyperplane_t*> hyperplane)
{
    int M = hyperplane.size();
    int D = hyperplane[0]->normal->dim;
    ...
    else
    {
        ...
        glp_delete_prob(lp); // clean up
        for(int e = 0; e < hyperplane.size(); e++)
        {
            double ddd = calculate_distance(hyperplane[e], feasible_pt);
            if(ddd < 0.01)
            {
                delete[]ia;
                delete[]ja;
                delete[]ar;
                release_point(feasible_pt);
                return NULL;
            }
        }
        delete[]ia;
