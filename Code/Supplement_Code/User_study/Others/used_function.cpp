...
/**
 * @brief           Check whether there are the same point as p in p_set
 * @param p         Point
 * @param p_set     Point set
 * @return          1 same
 *                  0 no same point existing
 */
...
void calculate_length(halfspace_set_t *R, double *len, double *position)
{
    ...
    for(int i = 0; i < R->ext_pts.size(); i++)
        print_point(R->ext_pts[i]);
    cout<<"\n";
    for(int i = 0; i <dim; i++)
        cout<< "position: "<< position[i] <<"  length: "<<len[i]<<"\n\n";
    */
}
...
void find_top1_sampling(std::vector<point_t *> p_set, std::vector<point_t *> &top_set, point_t *u, double segment, int level, int used_seg)
{
    ...
    else
    {
        for (int i = 0; i <= segment - used_seg; i++)
        {
            ...
            find_top1_sampling(p_set, top_set, u, segment, level + 1, used_seg + i);
        }
    }
    //printf("top1_size%d\n", p_set.size());
}
...
{
    int dim = p_set[0]->dim;
    int M = p_set.size(), original_size = p_set.size();
...
/**
 * @brief Check whether there are s top-k points w.r.t any utility vector in half_set
 * @param p_set			The dataset containing all the points
 * @param half_set 		The half_set/intersection of the halfspace
 * @param k 			top-k
 * @param s             The threshold of number
 * @param top_current 	The dataset containing all the certain top-k points found
 * @return              If there are s top-k points w.r.t any utility vector in half_set, return true
 *                      Otherwise, return false
 */
bool check_possible_s_topk(std::vector<point_t *> &p_set, halfspace_set_t *half_set, int k, int s, std::vector<point_t *> &top_current)
{
    int size = top_current.size(), index = 0;
    for (int i = 0; i < size; ++i)
    {
        int large_num = 0;
        for (int j = 0; j < p_set.size(); j++)
        {
            if (!is_same_point(p_set[j], top_current[index]))
            {
                hyperplane_t *h = alloc_hyperplane(top_current[index], p_set[j], 0);
                int relation = check_situation_positive(h, half_set);
                if (relation != 1)
                {
                    large_num++;
                    if (large_num >= k)
                    {
                        top_current.erase(top_current.begin() + index);
                        index--;
                        break;
                    }
                }
...
void print_interaction_result(FILE *wPtr, char *name, int k, int s, int numOfQuestion, vector<point_t*> &top_current, point_set_t *original_P)
{
    ...
    printf("------------------------------------------\n");
    printf("|%5s|%10s|%5s|%8s|%8s|\n", " Car", "Price(USD)", "Year", "PowerPS", "Used KM");
    printf("------------------------------------------\n");
    for(int w = 0; w < s; ++w)
    {
        printf("|%5d|%10.0f|%5.0f|%8.0f|%8.0f|\n", w + 1,
               original_P->points[top_current[w]->id]->coord[0],
               original_P->points[top_current[w]->id]->coord[1],
               original_P->points[top_current[w]->id]->coord[2],
               original_P->points[top_current[w]->id]->coord[3]);
    }
    printf("------------------------------------------\n");
    printf("Please give a number from 1 to 10 (i.e., 1, 2, .., 10) \n"
           "to indicate how bored you feel when you are asked with \n"
           "%d questions for this round in order to obtain %d out of \n"
           "your 20 most favorite cars (Note: 10 denotes you feel \n"
           "the most bored and 1 denotes you feel the least bored.)\n"
           "You answer: ", numOfQuestion, s);
    int sat = 0;
    while(sat < 1 || sat > 10)
      scanf("%d", &sat);
    printf("\n\n");
    ...
    fprintf(wPtr, "--------------------------------------------------------\n");
    fprintf(wPtr, "%s%s\n", "Algorithm: ", name);
    fprintf(wPtr, "%s %d \n", "The number of questions asked: ", numOfQuestion);
    fprintf(wPtr, "Recommended cars:\n");
    fprintf(wPtr, "--------------------------------------------------------\n");
    fprintf(wPtr, "|%3d%5s%2d|%10s|%10s|%10s|%10s|\n", s, " Top-", k, "Price(USD)", "Year", "PowerPS", "Used KM");
    fprintf(wPtr, "---------------------------------------------------------\n");
    for(int w = 0; w < s; ++w)
    {
    ...
    fprintf(wPtr, "---------------------------------------------------------\n");
    fprintf(wPtr, "Degree of boredness %d\n\n\n", sat);
}
...
void algorithm_comparison(FILE *wPtr, point_set_t *P, double *num, vector<point*> *res, int k)
{
    ...
    for(int s = 1; s <= k; ++s)
    {
        if(s == 1 || s == 5 || s == 10 ||s == 15 || s == 20)
        {
            index++;
            if (s == 1)
                printf("\n========================= Result %d (%d car shown in the output) ==========================\n", index, s);
            else
                printf("\n========================= Result %d (%d cars shown in the output) =========================\n", index, s);
            printf("The number of questions asked: %.0f \n", num[s - 1]);
            printf("Recommended cars:\n");
            printf("------------------------------------------     ------------------------------------------\n");
            printf("|%5s|%10s|%5s|%8s|%8s|     |%5s|%10s|%5s|%8s|%8s|\n", " Cars", "Price(USD)", "Year", "PowerPS",
                   "Used KM", " Cars", "Price(USD)", "Year", "PowerPS", "Used KM");
            printf("------------------------------------------     ------------------------------------------\n");
            for (int j = 0; j < s; ++j)
            {
                printf("|%5d|%10.0f|%5.0f|%8.0f|%8.0f|", j + 1,
                       P->points[res[s - 1][j]->id]->coord[0],
                       P->points[res[s - 1][j]->id]->coord[1],
                       P->points[res[s - 1][j]->id]->coord[2],
                       P->points[res[s - 1][j]->id]->coord[3]);
                if (j % 2 == 1)
                    printf("\n");
                else
                    printf("     ");
    ...
    printf("Please give your preferences on these results where the result you like the most is placed at \n"
           ...
           "to their need.\n"
           "(Result 1)  %.0f questions asked && 1 car shown in the output;  (Result 2)  %.0f questions asked && 5 cars shown in the output;\n"
           "(Result 3) %.0f questions asked && 10 cars shown in the output; (Result 4)  %.0f questions asked && 15 cars shown in the output;\n"
           "(Result 5) %.0f questions asked && 20 cars shown in the output; \n"
           "Your answer: ", num[0], num[4], num[9], num[14], num[19]);

    int rank[5];
    for(int i = 0; i < 5; i++)
        cin >> rank[i];
    fprintf(wPtr, "Rank: %d %d %d %d %d\n\n", rank[0], rank[1], rank[2], rank[3], rank[4]);
}