...
int STMD(FILE *wPtr, std::vector<point_t *> p_set, point_set_t *P0, double theta, int &questions, int alg_num)
{
    ...
    int new_size = floor(2*p_set_1.size() * theta);
    ...
    // printf("---------------------------------------------------------\n");
    print_result(wPtr, "STMD", numOfQuestion, P0->points[point_result->id], alg_num);
