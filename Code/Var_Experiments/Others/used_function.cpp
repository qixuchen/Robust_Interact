...
void hyperplane_random(std::vector<hyperplane_t *> &h_set)
{
    ...
    {
        int n = ((int) rand()) % M;
        hyperplane_t *pp = h_set[n];
        h_set.erase(h_set.begin() + n);
        h_set.push_back(pp);
    }
}
...
point_set_t * point_reload_delete_same(std::vector<point_t *> p_set)
{
    ...
    //reinsert
    M = p0.size();
    point_set_t* point_set = alloc_point_set(M);
    for (int i = 0; i < M; i++)
    {
        point_set->points[i] = p0[i];
    }
    p0.clear(); 
    //p0.shrink_to_fit();
    return point_set;
...
bool exist_same_hyperplane(std::vector<hyperplane_t*> h_set, hyperplane* h)
{
    ...
    for(int i=0; i<M; i++)
    {
        if(is_same_point(h_set[0]->normal, h->normal))
        {
            return true;
        }
    }
...
/**
 * @brief Used to check if top k point can be determined. 
 *          If yes, return the top k result in top_current
 *          If not, return a pair of points that can be used to partition the utlity range in point_pair
 *
 * @param p_set			The dataset containing all the points
 * @param half_set 		The half_set/intersection of the halfspace
 * @param k 			top-k
 * @param top_current 	The dataset containing all the possible top-k point found by function find_top_k_point_by_ext()
 * @param point_pair    if top k cannot be determined, this pair of point can partition the utility range
 * @return              If there is a top-k point w.r.t any utility vector in half_set, return it
 *                      Otherwise, return false
 */
bool checkTopkDetermined(std::vector<point_t *> p_set, halfspace_set_t *half_set, int k, 
                        std::vector<point_t *> &top_current, std::vector<point_t *> & point_pair){
    top_current.clear();
    point_pair.clear();
    bool top_k_determined = true;

    int M = half_set->ext_pts.size();
    if (M <= 1)
    {
        printf("%s\n", "None of the ext_pts in the set.");
        return top_k_determined;
    }

    int D = half_set->ext_pts[0]->dim;
    
    std::vector<point_t *> top_first_ext, top_other_ext;

    find_top_k(half_set->ext_pts[0], p_set, top_first_ext, k); // first find top-k for one of ext pt
    
    for(int i=1; i<M; i++){
        top_other_ext.clear();
        find_top_k(half_set->ext_pts[i], p_set, top_other_ext, k);

        // compare the two top k, if they disagree, find a pair of points for asking user
        if(top_first_ext.size()!=top_other_ext.size()){
            top_k_determined = false;
            point_pair.push_back(top_first_ext[top_first_ext.size()-1]);
            point_pair.push_back(top_other_ext[top_other_ext.size()-1]);
        }
        else{ // the two vectors have equal size
            for(int j=0;j<top_first_ext.size();j++){
                if(!is_same_point(top_first_ext[j],top_other_ext[j])){
                    top_k_determined = false;
                    point_pair.push_back(top_first_ext[j]);
                    point_pair.push_back(top_other_ext[j]);
                    break;
                }
            } 
        }
        // hyperplane_t *hp=alloc_hyperplane(point_pair[0], point_pair[1],0);
        // bool pos = check_situation_positive(hp,half_set->ext_pts);
        if( top_k_determined==false ){
            break;
        }
    }

    if(top_k_determined == true){
        top_current = top_first_ext;
    }
    return top_k_determined;
}
