...
int Active_Ranking_Alltopk(std::vector<point_t *> p_set, point_t *u, double theta, int output_size, std::vector<point_t*> ground_truth)
{
    timeval t1, t2;
    gettimeofday(&t1, 0);
    int dim = p_set[0]->dim;
    ...
    for(int i = 0; i < output_size; ++i)
        printf("|%30s |%10d |%10d |\n", "Point", i + 1, current_use[i]->id);
    printf("---------------------------------------------------------\n");

    top_k_correct=1;
    for(int i = 0; i < output_size; i++){
        if(current_use[i]->id != ground_truth[i]->id){
            top_k_correct=0;
            break;
        }
    }
